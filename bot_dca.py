...
class FastDCABot:
    def __init__(self):
        ...
        # Добавляем повторные попытки при ошибках подключения
    ...
    async def set_symbol_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Начать выбор токена"""
        logger.info("set_symbol_start called")
        ...
        await update.message.reply_text(
            ...
            f"Текущий: {self.db.get_setting('symbol', 'TONUSDT')}",
    ...
    async def process_symbol_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "✏️ Ввести свой токен":
            await update.message.reply_text(
                "✏️ Введите символ токена (например: TONUSDT, BTCUSDT, ETHUSDT):",
                reply_markup=self.get_cancel_keyboard()
            )
            return SET_SYMBOL_MANUAL
        
        # Проверяем выбранный из списка токен
        if text in POPULAR_SYMBOLS:
            return await self._validate_and_set_symbol(update, text)
        else:
            await update.message.reply_text(
                "❌ Неверный выбор. Пожалуйста, выберите токен из списка или введите свой:",
                reply_markup=self.get_symbol_selection_keyboard()
            )
            return SELECTING_SYMBOL
    ...
    async def _validate_and_set_symbol(self, update: Update, symbol: str) -> int:
        ...
        if not price:
            await update.message.reply_text(
                f"❌ Символ {symbol} не найден на Bybit. Попробуйте другой:",
                reply_markup=self.get_symbol_selection_keyboard()
            )
            return SELECTING_SYMBOL
        ...
        # Если токен изменился, сбрасываем статистику
        if old_symbol != symbol:
            self.db.set_setting('last_purchase_price', '0')
            self.db.set_setting('last_purchase_time', '0')
            # Не удаляем покупки, они остаются в базе для истории
    ...
    async def import_database_receive(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not update.message.document:
            await update.message.reply_text(
                "❌ Пожалуйста, отправьте файл .json или нажмите Отмена",
                reply_markup=self.get_cancel_keyboard()
            )
            return WAITING_IMPORT_FILE
    ...
    async def edit_purchase_selected(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Проверяем, является ли сообщение кнопкой редактирования
        if text in ["💰 Изменить цену", "📊 Изменить количество", "📅 Изменить дату", "❌ Удалить покупку", "🔙 Назад к списку"]:
            # Эти кнопки обрабатываются в других состояниях
            return EDIT_PURCHASE_SELECT
        
        try:
            # Извлекаем ID из текста кнопки (формат: "ID123: ...")
            import re
        ...
        edit_purchases_conv = ConversationHandler(
            ...
            states={
                EDIT_PURCHASE_SELECT: [
                    ...
                    MessageHandler(filters.TEXT & ~filters.COMMAND, self.edit_purchase_selected),  # Обработка выбора покупки
                ],
    ...
    def run(self):
        ...
        print(f"{Fore.WHITE}💾 База данных: dca_bot.db (сохраняется при обновлении)")
        print(f"{Fore.CYAN}{'='*60}\n")
        ...
        try:
            ...
            print(f"{Fore.YELLOW}⚠️ Проверьте подключение к интернету и доступ к api.telegram.org")
