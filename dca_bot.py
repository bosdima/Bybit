...
class FastDCABot:
    ...
    def get_settings_keyboard(self):
        ...
        keyboard = [
            ...
            [KeyboardButton("🔔 Уведомления о цене")],
    ...
    async def toggle_price_alert(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Включить/выключить уведомления"""
        current = self.db.get_setting('price_alert_enabled', 'false')
        new_value = 'false' if current == 'true' else 'true'
        self.db.set_setting('price_alert_enabled', new_value)
        
        status = "включены" if new_value == 'true' else "выключены"
        await update.message.reply_text(f"🔔 Уведомления {status}", reply_markup=self.get_settings_keyboard())
        return SELECTING_ACTION
        ...
        settings_conv = ConversationHandler(
            ...
            states={
                SELECTING_ACTION: [
                    ...
                    MessageHandler(filters.Regex('^(🔔 Уведомления о цене)$'), self.toggle_price_alert),
